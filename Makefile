...
ENVARS = VASCO_PROXY|$(VASCO_PROXY),VASCO_REGISTRY|$(VASCO_REGISTRY),VASCO_STATUS|$(VASCO_STATUS),REDIS_ADDR|$(REDIS_ADDR),MINPORT|$(MINPORT),MAXPORT|$(MAXPORT),EXPECTED_SERVICES|$(EXPECTED_SERVICES),STATUS_TIME|$(STATUS_TIME),DISCOVERY_EXPIRATION|$(DISCOVERY_EXPIRATION),STATIC_PATH|$(STATIC_PATH),USE_SWAGGER|$(USE_SWAGGER)
...
build:
	...
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o build/$(PROJECT) -ldflags \
